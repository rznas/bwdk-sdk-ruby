  ...
  class Packing < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Packing` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Packing`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@price.nil? && @price > 2147483647
        invalid_properties.push('invalid value for "price", must be smaller than or equal to 2147483647.')
      end
    ...
    def valid?
      ...
      return false if !@price.nil? && @price > 2147483647
    ...
    def price=(price)
      ...
      if price > 2147483647
        fail ArgumentError, 'invalid value for "price", must be smaller than or equal to 2147483647.'
      end
